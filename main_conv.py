...
def train_step(model, optimizer, lr_scheduler, ds_iter, amp_scaler, training_config, model_config, writer, task, name):
    ...
    for step, batch in enumerate(epoch_iterator):    
        ...
        if (step + 1) % training_config["eval_frequency"] == 0 and len(mat_lst) == 0:
            is_attn = True
        else:
            is_attn = False
        if task == 'lra-retrieval':
        ...
        if transition == False:
            ...
            lr_scheduler.step(step-transition_step)
        losses.update(loss)
                ...
                if len(before_attn) == 0:
                    ...
                    if len(before_distance) == 0:
                        ...
                        if diff_cnt == model_config["num_layers"]:
                            ...
                            #opt.param_groups[0]['lr'] = training_config["learning_rate"]
                            #lr_scheduler.base_lrs=[training_config["learning_rate"]]
                        else:
                            for l in range(model_config["num_layers"]):
        ...
        if transition == True and len(mat_lst)==0:
            ...
            for l in range(model_config["num_layers"]):
                ...
                kernel = np.repeat(kernel[np.newaxis, :, :], 1, axis=0)
                kernel = torch.Tensor(kernel)
                kernel = torch.unsqueeze(kernel, 0)
                out = F.conv2d(torch.unsqueeze(data, 0), kernel, stride=1, padding='same')
                out = F.avg_pool2d(out,(block_size,block_size))
                out = out[0]
                attn_pattern = out.view(-1)
                topk = torch.topk(attn_pattern,int(attn_pattern.shape[0]*model_config['density']-num_blocks))[1]
                #print(topk)
                ...
                if type(model) == nn.DataParallel:
                    ...
                    transformer_module = getattr(model.model, module_name)
                    mha_module = getattr(transformer_module, "mha")
                    if task == 'lra-retrieval':
                        mha_module.reconstruct_for_blocksparse(model_config)
                    else:
